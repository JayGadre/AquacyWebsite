...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        aria-label="Sidebar"
        className={`fixed md:sticky top-0 md:top-4 left-0 md:left-4 h-screen md:h-[calc(100vh-32px)] glass-card hover:translate-y-0 rounded-r-2xl md:rounded-2xl flex flex-col transition-all duration-300 ease-in-out z-40 border-r border-[var(--glass-border)] shadow-[5px_0_30px_rgba(0,0,0,0.5)]
      ...
      >
        ...
        <button
          ...
          className="hidden md:flex absolute -right-3.5 top-6 z-50 w-7 h-7 rounded-full bg-slate-900 border border-[var(--glass-border)] items-center justify-center text-slate-300 hover:text-white hover:bg-[#0ea5e9] transition-all shadow-md"
        ...
        <div className={`flex items-center p-4 md:p-6 border-b border-[var(--glass-border)] h-[84px] relative overflow-hidden ${isCollapsed ? "justify-center" : "gap-3"}`}>
          ...
          <div className="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-primary/30 to-primary-hover/10 border border-primary/40 flex items-center justify-center shadow-[0_0_15px_var(--glass-glow)]">
        ...
        {/* Navigation */}
        <nav id="mobile-navigation" aria-label="Main Navigation" className="flex-1 py-8 px-3 space-y-2.5 overflow-y-auto">
          {navItems.map((item) => {
        ...
        {/* Sidebar Footer CTA */}
        <div className={`p-4 md:p-6 border-t border-[var(--glass-border)] bg-slate-950/40 ${isCollapsed ? "flex flex-col items-center" : ""}`}>
          {!isCollapsed ? (
            <>
